...
func TestReElection2A(t *testing.T) {
	...
	DPrintf(dLog2, "S%d reconnected", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no new leader should
	// be elected.
	DPrintf(dLog, "L%d and F%d disconnected", leader2, (leader2+1)%servers)
	...
	DPrintf(dLog2, "S%d reconnected", (leader2+1)%servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	DPrintf(dLog2, "S%d reconnected", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
